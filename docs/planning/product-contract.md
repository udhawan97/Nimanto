...
Hard-constraint flags include citizenship, clearance, work authorization, location, and role-specific transfer incompatibility. Affected jobs remain searchable. Each flag shows exact source text/locator, consequence (`visible_warning` or the one default `exclude_from_recommendations`), confidence, and a user path to correct the posting fact, profile fact, or preference without rewriting source history. Only a confirmed `no_sponsorship_of_any_kind` signal is excluded from recommended applications by default. Employment gaps never lower a score or create a blocker. The v0.1.0 free-text projection removes only documented cues and does not claim comprehensive de-identification; sensitive identity content must not be imported as scoring evidence.
