...
| Posting lifecycle | Schema version 10 stores source runs, immutable normalized observations, role availability, verification attempts, exact Match Publication Role hashes, role-wording reviews, and tenant-local reviewed employer entities/aliases. Each Role can inspect distinct source, observation, verification, and local timestamps plus its linked source-run completeness and bounded integrity hashes. Raw provider payloads are hashed and discarded under the current zero-hour policy. |
