...
Nimanto uses open-source packages under their own licenses. The release lockfile is the install authority; the exact v0.5.4 machine-readable inventories are the [CycloneDX SBOM](docs/releases/nimanto-v0.5.4.cdx.json) and [SPDX SBOM](docs/releases/nimanto-v0.5.4.spdx.json). Authoritative copyright and license texts remain with each package and its source distribution.
...
Some transitive optional/native packages expose alternative or weak-copyleft license choices. Consult the SBOM and the package's distributed license before redistributing a modified binary bundle. Nimanto v0.5.4 publishes source and does not redistribute a signed native binary.
