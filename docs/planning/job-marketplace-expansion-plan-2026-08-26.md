...
| H-1B boundary | Each role now exposes a three-layer evidence ledger: exact current-posting restrictions with locator/time and exact-snapshot candidate acknowledgement, current employer-authored policy when stored, and historical government evidence with provenance and limitations. The reviewed employer registry preserves exact alias source snapshots, keeps collisions ambiguous, and checksum-binds any trusted resolution evaluation. Newer Role content or Match Publications invalidate the acknowledgement path. Every result remains warning-only; no layer changes fit rank or becomes a legal conclusion. |
| Data control | `nimanto_export_v5` includes discovery profiles, source runs, observations, verification attempts, availability, exact Match Publication Role hashes, role-wording reviews, and reviewed employer entities/aliases; tenant deletion cascades through all new tenant-owned tables. |
