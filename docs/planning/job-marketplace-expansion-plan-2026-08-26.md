...
| Posting lifecycle | Schema version 12 stores source runs, immutable normalized observations, role availability, verification attempts, exact Match Publication Role hashes, role-wording reviews, tenant-local reviewed employer entities/aliases, and government provenance/language-review bindings. Each Role can inspect distinct source, observation, verification, and local timestamps plus its linked source-run completeness and bounded integrity hashes. Raw provider payloads are hashed and discarded under the current zero-hour policy. |
...
| ATS routing | `ats_routing_v1` exposes adapter-owned HTTPS links and exact recognized Greenhouse, Lever, or Ashby job paths. A separate candidate-requested `ats_verification_v1` action performs fixed-host detail checks for Greenhouse/Lever or a complete-board Ashby check, persists method-qualified outcomes, and never applies or follows a redirect. SmartRecruiters plus every unapproved discovery-origin link remains gated. |
| H-1B boundary | Each role now exposes a three-layer evidence ledger: exact current-posting restrictions with locator/time and exact-snapshot candidate acknowledgement, current employer-authored policy when stored, and historical government evidence with provenance and limitations. The reviewed employer registry preserves exact alias source snapshots, keeps collisions ambiguous, and checksum-binds any trusted resolution evaluation. Government import also requires an exact server-trusted archive/layout/reuse manifest, source row locator, and qualified-review manifest bound to the immutable source-specific language contract. Newer Role content or Match Publications invalidate the acknowledgement path. Every result remains warning-only; no layer changes fit rank or becomes a legal conclusion. |
| Data control | `nimanto_export_v7` includes discovery profiles, source runs, observations, verification attempts, availability, exact Match Publication Role hashes, role-wording reviews, reviewed employer entities/aliases, and government provenance/language-review manifests; tenant deletion cascades through all new tenant-owned tables. |
...
**Delivery status:** Phases 0–2 are implemented in the current source tree. Phase 3 now has a tested SmartRecruiters adapter, source schedules, per-role sanitized attribution/provenance cards, fail-closed direct ATS routing, and candidate-requested liveness verification for the three approved ATS sources. SmartRecruiters execution and redirect traversal from discovery feeds remain gated pending source-specific rights; Phase 3 is therefore not complete. Phase 4 cannot begin honestly without written feed rights and the named 14-day non-production bake-off. The code-owned Phase 5 foundation now includes exact Role-snapshot match hashes, exact-quote candidate acknowledgement, role-card evidence layers, the local checksum-addressed government-edition seam, a tenant-local reviewed employer entity/alias registry, registry-checksum-bound precision evaluation, fail-closed resolution, a server-trusted government archive/layout/reuse provenance manifest, and a source/transformation-bound qualified immigration-language review gate. Phase 5 is not complete until an approved reviewed alias set, a real official provenance manifest, and an actual qualified review manifest exist for an approved edition; any optional USCIS edition also requires manual verification before enablement.
...
The candidate may separately request `ats_verification_v1` for a ready route.
Greenhouse and Lever use their exact public detail endpoints; Ashby uses its
complete current employer-board response. The request follows no redirect,
retains no provider body, records one method-qualified Verification Attempt, and
does not create an Action Intent. A definitive detail `404` closes the Role. An
Ashby absence requires two complete checks at least six hours apart. A timeout,
rate limit, malformed/oversized response, partial board, or other provider
failure records `blocked` while preserving the last successful state.
...
**Implemented continuation:** ready Greenhouse, Lever, and Ashby routes expose a
candidate-controlled recheck. Detail and complete-list results update Role
Availability under their distinct closure rules; blocked attempts are durable
evidence and cannot turn into closure. SmartRecruiters execution and any
discovery-origin redirect/recheck remain closed behind source rights.
...
The implemented provenance gate stores the exact source page, archive/layout
checksums and versions, retrieval/data-as-of dates, row-set checksum, reuse
review, transformation, and immutable manifest checksum. It is infrastructure,
not approval of a DOL or USCIS edition.
The implemented language gate separately checksum-binds source-specific display
copy and prohibited conclusions to a named reviewer and qualification. The code
does not supply that human review or activate an official edition.
