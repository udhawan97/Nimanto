...
| Posting lifecycle | Schema version 9 stores source runs, immutable normalized observations, role availability, verification attempts, exact Match Publication Role hashes, and role-wording reviews. Each Role can inspect distinct source, observation, verification, and local timestamps plus its linked source-run completeness and bounded integrity hashes. Raw provider payloads are hashed and discarded under the current zero-hour policy. |
...
| H-1B boundary | Each role now exposes a three-layer evidence ledger: exact current-posting restrictions with locator/time and exact-snapshot candidate acknowledgement, current employer-authored policy when stored, and historical government evidence with provenance and limitations. Newer Role content or Match Publications invalidate the acknowledgement path. Every result remains warning-only; no layer changes fit rank or becomes a legal conclusion. |
| Data control | `nimanto_export_v4` includes discovery profiles, source runs, observations, verification attempts, availability, exact Match Publication Role hashes, and role-wording reviews; tenant deletion cascades through all new tenant-owned tables. |
