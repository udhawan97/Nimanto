...
status change cannot rewrite an earlier Submission Record. Schema version 15
records the topic and prompt on each Answer Block revision so a revision reads
correctly after its Answer Block is re-titled, and adds an internal manual-role
operation ledger that makes a repeated manual Role creation resolve to the Role
it already created. That ledger is internal bookkeeping and is not part of
export v10.
...
checksum, transformation version, complete trusted dataset-provenance manifest,
manifest checksum, exact language-review manifest/checksum, evaluation result,
and trusted evaluation provenance. The
manifest binds the HTTPS source page, archive and layout URLs/checksums, layout
...
language review, rather than receiving invented metadata or approval.
